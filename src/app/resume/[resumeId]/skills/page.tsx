  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Showcase your technical and soft skills</p>
      </div>

      <AISkillSuggest onAddSkills={handleAddSkills} existingSkills={existingSkillNames} />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {fields.length === 0 && (
          <div className="bg-white rounded-xl border border-dashed border-slate-300 p-12 text-center">
            ...
            <p className="text-sm text-slate-500 mb-4">Add your technical skills or use AI suggestions</p>
            <Button type="button" onClick={() => append({ name: '', level: 'Intermediate' })} variant="outline">
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex items-center gap-4">
            <div className="flex-1 space-y-2">
              <Label className="text-xs">Skill Name</Label>
              <Input {...register(`skills.${index}.name`)} placeholder="React.js" className="h-10" />
              {errors.skills?.[index]?.name && <p className="text-xs text-red-500">{errors.skills[index]?.name?.message}</p>}
            </div>
